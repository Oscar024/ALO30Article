...
function [Elite_antlion_fitness,Elite_antlion_position,Convergence_curve,bred,arquitecturasalo]=ALO(N,Max_iter,lb,ub,dim,fobj)
...
for i=1:size(antlion_position,1)
    [antlions_fitness(1,i),red]=fobj(antlion_position(i,:)); 
    arqui(i).nn = red;
end
...
while Current_iter<Max_iter+1
    ...
    for i=1:size(ant_position,1)  
        ...
        ant_position(i,:)=round((ant_position(i,:).*(~(Flag4ub+Flag4lb)))+ub.*Flag4ub+lb.*Flag4lb);  
        
        [ants_fitness(1,i),red]=fobj(ant_position(i,:));        
        arqui(i+N).nn = red;
    end
    ...
    arquitecturasalo(Current_iter).nn = bred;
    arquitecturasalo(Current_iter).best = Elite_antlion_fitness;
    arquitecturasalo(Current_iter).capas = Elite_antlion_position;
    arquitecturasalo(Current_iter).iteration = Current_iter;
